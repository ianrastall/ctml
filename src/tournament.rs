...
impl PartialDate {
    ...
    /// Days since the Unix epoch with unknown month/day filled in as `1`
    /// — "for grouping/sorting/overlap comparisons only," per
    /// `pgn_to_ctml.py::approx_date`'s own docstring; the real
    /// `PartialDate` (with its true precision) is what actually gets
    /// emitted. Uses Howard Hinnant's `days_from_civil` (public domain,
    /// widely used, correct across the whole proleptic Gregorian range —
    /// not hand-derived here, borrowed because getting a calendar
    /// day-count formula subtly wrong is exactly the kind of bug that
    /// stays invisible until a date lands near a leap-year boundary).
    pub fn approx_epoch_day(&self) -> i64 {
        let (y, m, d) = (self.y as i64, self.m.unwrap_or(1) as i64, self.d.unwrap_or(1) as i64);
...
pub(crate) fn is_valid_fide_id(s: &str) -> bool {
