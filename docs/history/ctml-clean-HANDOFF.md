...
Status: the Rust program's purpose is DECIDED, the repo is on GitHub
(`github.com/ianrastall/ctml-clean`). PGN import is now end to end:
`ctml-clean pgn-to-corpus` parses a PGN file, resolves every game's SAN
to UCI via the move generator, computes fingerprints, resolves
White/Black/Event/Site against the real player/event/place registries,
groups games into tournament occurrences, and writes a dedup-safe corpus
— merging into existing files on a rerun instead of duplicating, with
divergences logged rather than silently overwritten. Run against real
116,506-game and 252,136-game slices of `mega-database-2025-filtered.pgn`
(full corpus schema-validated: 3,381/3,381 valid) and confirmed
idempotent (a full rerun added 0 new files, 0 new games). See "PGN
import: tournament grouping, registry resolution, corpus writer" below.
Eight real commands exist total: `stats`, `diff-players`,
`ingest-crosstables`, `fingerprint-selftest`, `movegen-selftest`/`perft`,
`pgn-import`/`pgn-selftest`, `pgn-to-corpus`, and now
`dedup-crosstables`/`debug-dedup-pair` — cross-source tournament dedup
for `crosstables.json`, the last item from the project owner's original
three-part list. Two real bugs found and fixed by testing against real
data (see "Cross-source crosstable dedup" below): a roster-matching
format mismatch that silently prevented ANY chess-results-vs-other-source
match, and an `event_ref`-presence gate that silently prevented ANY
TWIC-vs-other-source match — together these meant the first working
version found **zero** cross-source duplicates in 48,198 real entries,
which was wrong, not a real property of the data. After both fixes: 12
confirmed multi-source merges, full corpus schema-validated
(20,188/20,189 — the one failure is the same pre-existing source-data
defect already documented under "ingest-crosstables", not new). Eleven
real commands exist total. `.gitignore` excludes the SSP/crosstables.json/
...
## PGN import: tournament grouping, registry resolution, corpus writer — this session

Six new modules, each a direct port of one file under
`D:\dev\proj\ctml\readers\`, read before writing — same discipline as
every other piece of this project:

- `src/playerindex.rs` ← `player_registry_index.py`: name-exact then
  surname-unique resolution over *every* player (not just FIDE-keyed
  ones — a PGN name has to resolve against the ~88,000 non-FIDE players
  too, so this is intentionally lighter-weight than `xmlplayers.rs`,
  which only exists to diff against the SSP).
- `src/eventindex.rs` ← `event_registry_index.py`: exact-ref, then
  name-date-overlap, then slug-prefix-date-overlap (the TWIC-vs-Mega-
  Database spelling-mismatch case) over `events.xml`'s 10,433
  `eventOccurrence` entries — a structure this repo's own `stats`
  command had never looked past `eventSeries` to see before now.
- `src/placeindex.rs` ← `place_registry_index.py`: name-exact over
  city-level places only.
- `src/tournamentgroup.rs` ← `pgn_to_ctml.py::group_tournaments`: groups
  by normalized (Event, Site), splits on a >21-day gap between
  consecutive dated games.
- `src/pgntournament.rs` ← `pgn_to_ctml.py::tournament_xml` /
  `person_name_xml` — note this reference function's name-splitting has
  **no suffix extraction**, unlike `names::split_name` (used by the SSP
  and crosstable converters). That's a real difference between two
  Python converters in the source project, not an oversight to
  "harmonize" here; replicated as-is.
- `src/corpus.rs` ← `corpus_writer.py::merge_tournament`: dedup-safe
  writer, slot identity `(white ref, black ref, round)`, divergent
  fingerprints logged and left untouched rather than overwritten.

**One deliberate implementation difference from the reference, not a
behavior change:** `corpus_writer.py`'s `find_matching_file` rescans and
re-parses every `*.xml` in the corpus directory on every single call —
confirmed by reading it, not assumed. Running the first version of the
Rust port against a 200MB/252K-game PGN slice made this concrete rather
than theoretical: it visibly stalled, 2,605 files written in over 6
minutes with no sign of finishing. Killed it, built `CorpusIndex` (an
in-memory `slug -> [(path, start_day, end_day)]` map, built once via
`CorpusIndex::build` and updated in place via `record` as files are
created or date-broadened) to replace the per-call disk rescan. Same
matching *rule*, same output — reran the identical 252K-game slice: full
run (indices + parse + import + group + write) in a few minutes instead
of stalling indefinitely, with the write phase alone dropping from
"6+ minutes, still not done" to 27.7s for 9,577 tournaments.

**Verified against real data, not just the self-generated PGN vectors**,
using two slices of `mega-database-2025-filtered.pgn`:

```
# 116,506 games (a 2016-era slice, chosen specifically because the
# first slice tried — 1821-era games — showed 0/9577 event matches,
# which needed distinguishing from a bug; see below)
imported 116506 games (0 failed)
grouped into 3382 tournaments
admitted 3382 of 3382 (floor 0): 3381 new files, 1 merged
player resolution: 14940/56392 (26.5%)
event resolution: 48/3382 (1.4%)
place resolution: 1068/3382 (31.6%)
```

- **Schema-validated the full corpus, not a sample**: 3,381/3,381 valid
  against `xsd/ctml.xsd`.
- **Idempotency, checked by actually rerunning, not assumed**: same
  116,506-game file, same corpus directory, second time: `0 new files, 0
  new games, 0 new participants, 116448 already present`. This is the
  actual point of a dedup-safe writer, and it was checked, not just
  implemented and trusted.
- **0/9577 event matches on the first (1821-era) slice was investigated,
  not shrugged off.** Reran against the 2016-era slice specifically to
  distinguish "the resolver is broken" from "this slice's data genuinely
  doesn't overlap the registry" — the second slice matched 48 events,
  confirming the resolver works and the first result was a real (if
  unsurprising) coverage gap: the registry's occurrences are
  OlimpBase/TWIC-sourced and don't meaningfully cover 1821-era friendly
  games.
- **58 divergences on the idempotent rerun, explained, not hand-waved.**
  All 58 traced to one recurring team-league event
  (`20170111-20170201-pro-league-central.xml`), each with a *different*
  white/black pair sharing `round="3"` — a real data-granularity limit:
  this team league reuses generic round labels across what are, in
  substance, different fixtures, and `(white, black, round)` — the exact
  identity model `corpus_writer.py` itself defines, not a Rust
  simplification of it — has no board-number component to disambiguate
  multiple same-round encounters between the same two nominal slot
  values. The divergence log did exactly its job: refused to silently
  overwrite, surfaced it instead.

**Explicit scope boundary, not yet done:** `--register-new-events`
(splicing newly-discovered event occurrences back into `events.xml`) —
`event_registry_writer.py` was read but not ported; the reference itself
defaults this off for the same reason it would need to be off here first
(TWIC/Mega-Database naming barely overlaps, so most "unresolved" events
are probably existing ones under a different spelling, not new).

## Cross-source crosstable dedup — this session

`src/crosstabledup.rs`, a port of `scripts/curate_source_tournaments.py`'s
clustering (`UnionFind`, `name_key`, `similarity`, `should_link`,
`cluster_facts`, `merged_table`), wired up as `ctml-clean
dedup-crosstables [assets-dir] [out-dir]` plus a diagnostic, `ctml-clean
debug-dedup-pair <idx-a> <idx-b>`, that traces `should_link`'s inputs for
two specific `crosstables.json` entries — built to debug this session's
own two bugs, kept because it's a real tool now, not a one-off.

**Adapted, not literally ported, and why.** That script's own
`DEFAULT_SOURCES` (`D:\ctml\build\drafts\{chess-results,twic,nwchess-
minev,olimpbase}`) turned out to only partially exist on disk (checked
directly: only `olimpbase\` is present), and the script targets the
superseded `urn:ctml:1.0` namespace besides. But the actual *data* it
clusters — event name, dates, participant roster — is exactly what's
already in this repo's `assets/crosstables.json`, so the clustering
*algorithm* runs unchanged against that instead of trying to resurrect a
directory tree that's gone.

**`difflib.SequenceMatcher.ratio()` was ported faithfully, not
approximated** — the Ratcliff/Obershelp algorithm (`find_longest_match`
+ recursive matching blocks), because `should_link`'s thresholds (0.94
name-only, 0.62 combined with roster overlap) were tuned against that
exact function; a rougher string-similarity substitute would silently
shift which pairs cluster. Skips `autojunk` (only ever activates for
sequences of 200+ elements; tournament-name strings never reach that).

**Two real bugs, found only because the first working version was run
against the real 48,198-entry file and produced a suspicious result (0
cross-source merges) that got investigated instead of accepted:**

1. **Roster-key format mismatch.** `player_roster_key` used one key per
   player — FIDE-id-based when available, else `name|fed` — matching the
   reference's own `roster_keys` design exactly. Checked directly against
   the real data: `twic` (96.4% of all 48,198 entries), `olimpbase`, and
   `nwchess-minev` carry a FIDE id on **0.0%** of their player rows;
   `chess-results` (a 211-entry minority, but the *highest*-priority
   source in `SOURCE_PRIORITY`) carries one on 75.4%. A chess-results
   entry and a TWIC entry describing the literal same tournament with the
   literal same players therefore produced completely disjoint roster-key
   sets — proven concretely with FIDE id 5th Gambit GM Closed
   (chess-results idx 46481) vs TWIC's capture of the same event (idx
   23725): 9/9 players in common by name, 0/9 by the single-key scheme.
   Fixed by comparing two independent channels (`roster_fide`,
   `roster_name` — every player contributes to `roster_name` regardless
   of FIDE availability) rather than one FIDE-preferred key per player,
   linking on whichever channel both sides actually share.
2. **`event_ref`-presence gate.** The reference's `if a.event_ref or
   b.event_ref: return False` (after the exact-match check) vetoes fuzzy
   matching the instant *either* side has a non-empty `event_ref`, matched
   or not. Checked directly: `event_ref` is populated for TWIC only
   (31,890 of its 46,472 entries) and *never* for the other three
   sources — so this gate doesn't express "these are known to disagree,"
   it expresses "TWIC resolved one locally and the other scraper never
   fills this field at all," and blocks essentially every legitimate
   TWIC-vs-other-source match category that exists in this corpus (TWIC
   being 96.4% of it). Confirmed against the same Gambit pair: still
   blocked here even after fix 1, `debug-dedup-pair` showing `b.event_ref`
   non-empty and `a.event_ref` empty. Fixed to veto only when *both* sides
   are resolved and actually differ; one side simply not having resolved
   one no longer counts as disagreement.

**Verified after both fixes, against the real file, not a sample:**

```
48198 source documents -> 20189 clusters
  9173 multi-entry merges, 11016 single-entry passthroughs
  12 multi-*source* merges
9858 entries dropped (no usable start date)
```

The 12 multi-source merges were checked by name, not just counted: `3rd
UzChess Cup Masters/Futures/Challengers`, `5th Gambit GM Closed
Tournament`, `54th Reggio Emilia`, `17th`/`20th Asian Continental`,
`7th Stepan Avagyan Memorial`, `Sinquefield Cup 2013`, `Battle of Minds
Challengers Edition`, `Vladimir Dvorkovich Memorial`, `Belgrade GM Swiss
Mix Tournament` — real tournament names, each a `{chess-results,twic}` or
`{olimpbase,twic}` pair. Schema-validated the full output, not a sample:
20,188/20,189 valid; the one failure
(`twic-438-19th-spring-fest-budapest-hun-hun-14-33-iii-2003.xml`, day=33)
is the exact same pre-existing source-data defect already documented
under "ingest-crosstables" above — same file, same root cause, not a new
issue introduced by clustering.

**Honest residual finding, not resolved:** 12 is almost certainly a
lower bound, not the true count — of 1,726 non-TWIC entries, 189 land in
the same `(year, first-name-token)` block as at least one TWIC entry
(checked directly), meaning many more *candidate* pairs exist than the 12
that actually crossed `should_link`'s thresholds. Whether the remainder
are genuinely distinct tournaments (the algorithm correctly declining to
merge) or further real duplicates the name/roster thresholds are still
too strict to catch is unknown — would need spot-checking specific
un-merged candidate pairs the way the two bugs above were found, not
guessed at.

## Full-scale production run — `pgn-to-corpus` against the complete 1.88M-game database

Everything above had only been run against slices (up to 252,136 games)
or `crosstables.json` (a different, smaller input). This is the first
run of the complete PGN pipeline — SAN resolution, registry lookups,
tournament grouping, dedup-safe corpus writing — against the entire real
production file, `min-elo=2000` (the reference's own default, not the
unfiltered `0` used for earlier slice testing):

```
parsed 1881897 games      49.91s
imported 1881897 games (0 failed)    801.81s
grouped into 52971 tournaments     11.89s
admitted 52971 of 52971 (floor 2000): 52743 new files, 228 merged
  (+9841 games, +2754 participants, 100 already present, 0 enriched, 14 divergences)
  0 rejected     221.11s
player resolution: 180138/836876 (21.5%)
event resolution: 547/52971 (1.0%)
place resolution: 17683/52971 (33.4%)
```

~18.5 minutes total, 7.7GB / 52,743 files written.

**`0 rejected` against a 2000 floor was checked, not assumed benign.**
Grepped every `WhiteElo` value in the source file directly: the minimum
anywhere in 1.88M games is 2300. The file's own name
(`mega-database-2025-filtered.pgn`) says "filtered" — it was pre-filtered
to a 2300+ rating floor upstream, before this pipeline ever saw it, so a
2000 floor here has nothing left to reject. The admission-floor code
itself is untested against a file that actually has low-rated games in
it — that's a real gap, not a claim this feature works end-to-end, only
that it didn't fire here because it had nothing to do.

**14 divergences, all the same already-understood pattern**, not a new
issue: team leagues/cups with year-only date precision (`19940000`
compact — `PCA Intel GP`, `ROM cht`, `RUS Cup`, `ISL cht`) and generic
recurring round labels, the identical root cause documented earlier under
"PGN import" for `pro-league-central` — `(white, black, round)` can't
distinguish multiple real fixtures that share a round number across a
season when the exact date isn't known.

**Schema-validated the complete output, not a sample: 52,743/52,743
valid, zero violations** (8m51s to check). This is the real, full-scale
confirmation the whole pipeline — not just each piece in isolation —
produces valid CTML from the actual production database end to end.

## CLI had no real help system — found by the project owner actually using it

Every session so far built commands and verified them work; nobody had
tried to just pick up the binary cold and figure out how to run it. The
project owner did, immediately: no args silently ran `stats` against
`./assets` with no explanation, and `--help` fell through to a single
crammed usage line (`ctml-clean: unknown command '--help'\nusage:
ctml-clean <stats|diff-players|...>`) — a bare list of 9 of the 11
command names (`debug-dedup-pair` wasn't even in it), no arguments, no
defaults, no description of what any of them do. Exactly the same class
of gap as the original "Hello, world" problem this project started
from — real functionality existed, but nothing told you it was there or
how to use it.

Fixed: no args, `-h`, or `--help` now print a categorized command list
(read-only diagnostics / self-tests / real pipelines / other
diagnostics) with a one-line summary each; `ctml-clean help <command>` or
`ctml-clean <command> --help` prints that command's full usage —
arguments, defaults, and a real description, written from what each
command's own code actually does, not copy-pasted from memory. An
unknown command now prints the same top-level list instead of a bare
error. All four entry points (no-args, `--help`, `help <cmd>`, `<cmd>
--help`) were actually run and their output read, not just assumed to
work from the code — including confirming ordinary command dispatch
(`perft`, `fingerprint-selftest`) still runs correctly after the
`main()` restructuring this needed.

## Next real unit of work

All three items from the project owner's original list are done and now
verified at full production scale, not just on slices. Two known,
explicit gaps if picked back up: the admission floor has never actually
rejected anything (needs a source file with real low-rated games to
verify it works, not just that it compiles); the "12 multi-source merges
is likely a lower bound" question from the crosstables dedup section
above is still open. Otherwise not yet decided what's next — say what to
pick up.
